...
const FARM_LOCATIONS = [
    ...
    {
        id: 5,
        title: "Destination", // Northern Field
        coordinates: { latitude: 24.9199993, longitude: 67.3011876 },
        image: require('../assets/images/ndvi.jpg'),
        description: "فصل کی صحت کی تصویر", // Crop Health Image
    },
    {
...
const FARM_BOUNDARY = [
    { latitude: 24.9194751, longitude: 67.3010683 },
    { latitude: 24.9196089, longitude: 67.3014421 },
    { latitude: 24.9200644, longitude: 67.3012151 },
    { latitude: 24.9199546, longitude: 67.3008748 },
    { latitude: 24.9194751, longitude: 67.3010683 }, // Close the polygon
];
...
export default function MapScreen() {
    ...
    const [droneDestination, setDroneDestination] = useState<{ latitude: number, longitude: number } | null>(null);
    const [isDroneMoving, setIsDroneMoving] = useState(false);
    const [droneStartPosition, setDroneStartPosition] = useState<{ latitude: number, longitude: number } | null>(null);
    ...
    // Replace the existing drone movement useEffect with this implementation
    useEffect(() => {
        let droneInterval: NodeJS.Timeout;

        if (isDroneMoving && droneDestination) {
            // Movement speed factors - adjust these to control drone speed
            const movementSpeed = 0.000001; // Movement increment per tick
            const intervalTime = 100; // Update interval in ms (higher = slower)

            droneInterval = setInterval(() => {
                setDrone(prevDrone => {
                    // Calculate direction vector
                    const latDiff = droneDestination.latitude - prevDrone.latitude;
                    const lngDiff = droneDestination.longitude - prevDrone.longitude;
                    ...
                    if (distance < movementSpeed) {
                        ...
                        return {
                            ...prevDrone,
                            latitude: droneDestination.latitude,
                            longitude: droneDestination.longitude,
                    ...
                    const newLat = prevDrone.latitude + normalizedLat * movementSpeed;
                    const newLng = prevDrone.longitude + normalizedLng * movementSpeed;

                    // Update battery level based on movement
                    const newBattery = prevDrone.battery > 0 ? prevDrone.battery - 0.005 : 0;
        ...
        // Clean up interval on unmount or when movement stops
        return () => {
            if (droneInterval) {
                clearInterval(droneInterval);
            }
        };
    }, [isDroneMoving, droneDestination]);



    const sendDroneToLocation = (destination: { latitude: number, longitude: number }) => {
        setDroneStartPosition({
            latitude: drone.latitude,
            longitude: drone.longitude
        });
        setDroneDestination(destination);
                                        ...
                                        <>
                                            ...
                                            <Button
                                                ...
                                                onPress={() => {
                                                    ...
                                                    sendDroneToLocation(selectedLocation.coordinates);
